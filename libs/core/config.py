...
DEFAULTS = {
    ...
    "test_cfg": {
        "pre_nms_thresh": 0.001,    # inference 过滤置信度低的预测！！
        "pre_nms_topk": 5000,
        "iou_threshold": 0.1,   # p.p. 重叠阈值
        "min_score": 0.01,  # p.p. 过滤的得分低的预测！！
        "max_seg_num": 1000,    # p.p. 最大预测片段数量
        "nms_method": 'soft', # soft | hard | none
        "nms_sigma" : 0.5,  # p.p. 控制得分衰减
        "duration_thresh": 0.05,    # inference 过滤持续时间段的预测
        "multiclass_nms": True,
        "ext_score_file": None,
        "voting_thresh" : 0.75, # p.p. 是否合并阈值
    },
...
def _update_config(config, training):
    if config["dataset"]["task"] in ['zero_shot', 'gzs']:
        if training:
            config["dataset"]["num_classes"] = config["dataset"]["train_num_classes"]
        else:
            config["dataset"]["num_classes"] = config["dataset"]["test_num_classes"]
...
def load_config(config_file, training, defaults=DEFAULTS):
    with open(config_file, "r") as fd:
        config = yaml.load(fd, Loader=yaml.FullLoader)
    _merge(defaults, config)
    config = _update_config(config, training)
    return config