...
def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0, label_dict=None, 
                          feat_folder = None, file_prefix = None, file_ext = None, ):
    # load json file
    ...
    for k, v in json_db.items():
        ...
        if (split is not None) and v['subset'].lower() != split:
            continue
        # 跳过没有feat文件的视频 InternVideo
        if file_prefix is None:
            file_prefix = ''
        if not os.path.exists(os.path.join(feat_folder, file_prefix + k + file_ext)):
            continue
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.ground_truth = load_gt_seg_from_json(
            ant_file, split=self.split, label=label, label_offset=label_offset, label_dict=label_dict, feat_folder=feat_folder, file_prefix=file_prefix, file_ext=file_ext, )
    ...
    def wrapper_compute_average_precision_part(self, preds, processed_ground_truth):
        """Computes average precision for each class in the subset.
        """
        ap = np.zeros((len(self.tiou_thresholds), len(self.activity_index)))

        # Adaptation to query faster
        ground_truth_by_label = processed_ground_truth.groupby('label')
        prediction_by_label = preds.groupby('label')
    ...
    def wrapper_compute_topkx_recall_part(self, preds, processed_ground_truth):
        """Computes Top-kx recall for each class in the subset.
        """
        recall = np.zeros((len(self.tiou_thresholds), len(self.top_k), len(self.activity_index)))
    ...
    def evaluate_part(self, preds, processed_video_ids=None, verbose=True):
        """Evaluates a prediction file. For the detection task we measure the
        interpolated mean average precision to measure the performance of a
        method.
        preds can be (1) a pd.DataFrame; or (2) a json file where the data will be loaded;
        ...
        if processed_video_ids is not None:
            processed_ground_truth = self.ground_truth[self.ground_truth['video-id'].isin(processed_video_ids)]

        # compute mAP
        if processed_ground_truth is not None:
            self.ap = self.wrapper_compute_average_precision_part(preds, processed_ground_truth)
            self.recall = self.wrapper_compute_topkx_recall_part(preds, processed_ground_truth)
        else:
            self.ap = self.wrapper_compute_average_precision(preds)
            self.recall = self.wrapper_compute_topkx_recall(preds)
        mAP = self.ap.mean(axis=1)
