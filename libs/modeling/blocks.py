...
# from .ttt_model.ttt_encoder_s import TTT_Encoder_S
# from .ttt_model.ttt_encoder_s1 import TTT_Encoder_S1
...
## Attn替换TTT【保留FFN】
class TransformerBlock_TTT(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
    def __init__(
        ...
        # single_TTT / double_TTT
        self.ttt_type = ttt_type
        if self.ttt_type == 'un_ttt':
            self.ttt_layer = TTT_Encoder_Un(nhead=8, num_layers=1, d_model=n_embd, use_cache=True, mini_batch_size=mini_batch_size, ttt_layer_type='mlp', window_size=window_size)
        
        elif self.ttt_type == 'bi_ttt':
            self.bi_ttt_type = bi_ttt_type
            if self.bi_ttt_type == 'single':
                #self.ttt_layer = TTT_Encoder_S1(nhead=8, num_layers=1, d_model=512, use_cache=True, mini_batch_size=64, ttt_layer_type='mlp')
                self.ttt_layer = TTT_Encoder_Bi_S(nhead=8, num_layers=1, d_model=n_embd, use_cache=True, mini_batch_size=mini_batch_size, ttt_layer_type='mlp', window_size=window_size, ar_pred=ar_pred)
            elif self.bi_ttt_type == 'double':
                #self.ttt_layer = TTT_Encoder_S(nhead=8, num_layers=1, d_model=512, use_cache=True, mini_batch_size=64, ttt_layer_type='mlp')
                self.ttt_layer = TTT_Encoder_Bi_D(nhead=8, num_layers=1, d_model=n_embd, use_cache=True, mini_batch_size=mini_batch_size, ttt_layer_type='mlp', window_size=window_size, ar_pred=ar_pred)
    ...
    def forward(self, x, mask, pos_embd=None):
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf
        x_norm = self.ln1(x)
        ## TTT_Encoder: MaskedConv1D + TTT_layer
        # MaskedConv1D -- in: B x 512 x T_i / out: B x 512 x T_i/2
        out, out_mask = self.value_conv(x_norm, mask)
        # TTT_layer -- in: B x 512 x T / out: B x 512 x T
        #out = self.ttt_layer(out.permute(0, 2, 1)).permute(0, 2, 1)  # (B, 512, T_i)  # no mask
        out = self.ttt_layer(out.permute(0, 2, 1), out_mask.permute(0, 2, 1)).permute(0, 2, 1)  # (B, 512, T_i)
...
## Attn替换TTT【不保留FFN】
class TTT_Encoder_Block(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
    def __init__(
        ...
        if self.ttt_type == 'un_ttt':
            ...
            if self.bi_ttt_type == 'single':
                #self.ttt_layer = TTT_Encoder_S1(nhead=8, num_layers=1, d_model=512, use_cache=True, mini_batch_size=64, ttt_layer_type='mlp')
                self.ttt_layer = TTT_Encoder_Bi_S(nhead=8, num_layers=1, d_model=n_embd, use_cache=True, mini_batch_size=mini_batch_size, ttt_layer_type='mlp', window_size=window_size)
            elif self.bi_ttt_type == 'double':
                #self.ttt_layer = TTT_Encoder_S(nhead=8, num_layers=1, d_model=512, use_cache=True, mini_batch_size=64, ttt_layer_type='mlp')
                self.ttt_layer = TTT_Encoder_Bi_D(nhead=8, num_layers=1, d_model=n_embd, use_cache=True, mini_batch_size=mini_batch_size, ttt_layer_type='mlp', window_size=window_size, ar_pred=ar_pred)
    ...
    def forward(self, x, mask, pos_embd=None):
        ...
        # MaskedConv1D -- in: B x 512 x T_i / out: B x 512 x T_i/2
        out, out_mask = self.value_conv(x_norm, mask)
        # TTT_layer -- in: B x T x 512 / out: B x 512 x T
        #out = self.ttt_layer(out.permute(0, 2, 1)).permute(0, 2, 1)  # (B, 512, T_i)  # no mask
        out = self.ttt_layer(out.permute(0, 2, 1), out_mask.permute(0, 2, 1)).permute(0, 2, 1)  # (B, 512, T_i)
...
## Trans_Encoder插入TTT（不同位置ttt_pos）
class Trans_TTT_Block(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
        ...
        if ttt_pos != -1:
            ...
            if self.ttt_type == 'un_ttt':
                ...
                if self.bi_ttt_type == 'single':
                    ...
                    #self.ttt_layer = TTT_Encoder_S(nhead=8, num_layers=1, d_model=512, use_cache=True, mini_batch_size=64, ttt_layer_type='mlp')
                    self.ttt_layer = TTT_Encoder_Bi_D(nhead=8, num_layers=1, d_model=n_embd, use_cache=True, mini_batch_size=mini_batch_size, ttt_layer_type='mlp', window_size=window_size)
    ...
    def forward(self, x, mask, pos_embd=None):
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf
        # Attn -- in: B x 512 x T_i / out: B x 512 x T_i/2
        x_norm = self.ln1(x)
        if self.ttt_pos == 0:
            _, out_mask = self.value_conv(x_norm, mask)
            x_norm = self.ttt_layer(x_norm.permute(0, 2, 1), out_mask.permute(0, 2, 1)).permute(0, 2, 1)
        out, out_mask = self.attn(x_norm, mask)
        out_mask_float = out_mask.to(out.dtype)
        # TTT -- in: B x T x 512 / out: B x 512 x T
        if self.ttt_pos == 1:
            out = self.ttt_layer(out.permute(0, 2, 1), out_mask.permute(0, 2, 1)).permute(0, 2, 1)
        out = self.pool_skip(x) * out_mask_float + self.drop_path_attn(out)
        # FFN
        x_norm2 = self.ln2(out)
        if self.ttt_pos == 2:
            x_norm2 = self.ttt_layer(x_norm2.permute(0, 2, 1), out_mask.permute(0, 2, 1)).permute(0, 2, 1)
        out = out + self.drop_path_mlp(self.mlp(x_norm2) * out_mask_float)
        # optionally add pos_embd to the output
        if pos_embd is not None:
            out += pos_embd * out_mask_float
        return out, out_mask
