...
class VideoIntern(nn.Module):
    ...
    def myforward(self, video_feat, text):
        
        # if video_feat.shape == 3:
        # video_feat B x T_i x 768
        # text_feat = self.encode_text(text)  # -> num_classes x 768
        text_feat = text
        
        # normalized features
        epsilon = 1e-8
        video_feat = video_feat / (video_feat.norm(dim=1, keepdim=True)+epsilon)
        text_feat = text_feat / text_feat.norm(dim=1, keepdim=True)

        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        # logits_per_video = logit_scale * video_feat @ text_feat.t()
        logits_per_video = torch.einsum("bqd,cd->bqc",video_feat,text_feat)*logit_scale
