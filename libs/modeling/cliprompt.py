...
class CLIPrompt(torch.nn.Module):
    def __init__(self, actionlist, actiondict, actiontoken, device):
    ...
    def myforward(self, inp_actionlist):
        self.replace_text_embedding(inp_actionlist)
        tFeature = self.clipmodel.encode_text(self.text_embedding.half(), self.prompt_actiontoken.half())
