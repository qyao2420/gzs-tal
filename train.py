...
def main(args):
    ...
    if os.path.isfile(args.config):
        cfg = load_config(args.config, training=True)
    else:
    ...
    subset_file = data_split_dir(cfg['dataset']['subset_file'], cfg['dataset']['task'], cfg['dataset']['data_split'], 'train', cfg['dataset']['split_num'])
    ...
    #model = load_model_ckpt(model, 'ckpt/thumos_clip_clip_bittt_d_16/epoch_050.pth.tar', cfg['devices'][0])
...
def load_model_ckpt(model, ckpt_file, device):
    # load ckpt, reset epoch / best rmse
