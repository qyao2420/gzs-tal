## 用于连续eval每一epoch的model，并获取指标AVG最高的epoch
...
def main(args):
    ...
    avg_values = []
    draw_plt = True
    plt_path_png = f'./plts/aa.png'
    plt_path_svg = f'./plts/aa.svg'

    min_epoch = args.epoch
    max_epoch = 205
    step = 5

    for i in range(min_epoch, max_epoch+1, step):
        ...
        ckpt_file = os.path.join(args.ckpt, 'epoch_{:03d}.pth.tar'.format(i))
        ...
        avg_values.append(mAP)
        if mAP > max_avg:
            max_avg = mAP
            best_epoch = i

    print(f'Best_AVG: {max_avg} [Best_Epoch: {best_epoch}]')
    '''# 绘制 epoch-AVG 折线图
    if draw_plt:
        draw_plt_method(min_epoch, max_epoch, step, avg_values, plt_path_png, plt_path_svg)'''
    return

'''def draw_plt_method(min_epoch, max_epoch, step, avg_values, plt_path_png, plt_path_svg):
    
    plt.figure(figsize=(10, 5))
    plt.plot(range(min_epoch, max_epoch+1, step), avg_values, marker='o', linestyle='-', color='b')
    plt.title(f'Map Values Over Epochs ({min_epoch} to {max_epoch})')
    plt.xlabel('Epoch')
    plt.ylabel('Map Value')
    plt.xticks(range(min_epoch, max_epoch+1, step))
    plt.grid()

    plt.savefig(plt_path_png)
    plt.savefig(plt_path_svg)
    plt.close()
